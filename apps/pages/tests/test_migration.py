...
# --- migration_review_view tests ---


@pytest.mark.django_db
class TestMigrationReviewView:
    def test_anonymous_redirected(self, client):
        response = client.get("/gestion/migration/")
        assert response.status_code == 302
        assert "comptes" in response.url or "login" in response.url
    ...
    def test_admin_can_access(self, client, admin_user):
        # Without inventory file, redirects to home
        client.force_login(admin_user)
        response = client.get("/gestion/migration/")
        assert response.status_code == 302  # redirect because no inventory

    def test_mayor_can_access(self, client, mayor):
        client.force_login(mayor)
        response = client.get("/gestion/migration/")
        assert response.status_code == 302  # redirect because no inventory

    def test_review_with_inventory(self, client, admin_user):
        inventory = [
            {
                "source_url": "/test.html",
                "source_title": "Test",
    ...
    def test_review_saves_decisions(self, client, admin_user):
        inventory = [
            {
                "source_url": "/test.html",
                "source_title": "Test",
        ...
        try:
            ...
            response = client.post(
                "/gestion/migration/",
                {
                    "status_0": "mettre_a_jour",
                    "target_0": "commune/test",
                    "notes_0": "À vérifier",
                },
            )
            assert response.status_code == 200
            ...
            assert decisions[0]["final_status"] == "mettre_a_jour"
            assert decisions[0]["final_target"] == "commune/test"
            assert decisions[0]["final_notes"] == "À vérifier"
        finally:
            inventory_path.unlink(missing_ok=True)
            decisions_path.unlink(missing_ok=True)
